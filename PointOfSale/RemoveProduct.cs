...
namespace PointOfSale
{
    public partial class RemoveProduct : Form
    {
        ...
        public RemoveProduct(DataGridView dgv)
        {
            InitializeComponent();
            this.mDgv = dgv;
            CommandLine commandLine = new CommandLine("RemoveProduct");
        }
        ...
        {
            DialogResult dialogResult = MessageBox.Show("Are you Sure delete product details?", "Veryfy", MessageBoxButtons.YesNo);
            ...
            if (dialogResult == DialogResult.Yes)
            {
                ...
                DeleteDatabaseColumn.company_id = connector.SearchCompanyList(TempCompanyList, cmbCompanyName.SelectedItem.ToString()).company_id;
                DeleteDatabaseColumn.category_id = connector.SearchCategoryList(TempCategoryList, cmbCategoryName.SelectedItem.ToString()).category_id;
                DeleteDatabaseColumn.product_id = txtProductId.Text.ToString();
                DeleteDatabaseColumn.product_name = txtProductName.Text;
                DeleteDatabaseColumn.sell_price = txtSellPrice.Text;
                DeleteDatabaseColumn.product_discount_price = txtDiscount.Text;
                DeleteDatabaseColumn.minimum_quantity = txtMinimumQuantity.Text;

                List<DatabaseColumn> TempListForDGV = new List<DatabaseColumn>();
                TempListForDGV.Add(DeleteDatabaseColumn);


                connector.establish();
                connector.SetData(DeleteDatabaseColumn);

                if (connector.RemoveProductDetails() == true)
                {
        ...
        private void RemoveProduct_Shown(object sender, EventArgs e)
        {
            getCompanyList();
            getCategoryList();
